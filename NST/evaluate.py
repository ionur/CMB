...
gen_net.load_state_dict(torch.load('./models/gennet2.pt'))
...
with torch.no_grad():
    for i, (X, _, Y) in enumerate(loader):
        X = X.to(device)
        out = gen_net(X.to(device))
        # out = (out - torch.mean(out,dim=[1,2,3],keepdim=True))/torch.std(out,dim=[1,2,3],keepdim=True)
        out = out.squeeze()
        Y = Y.squeeze()
